...
import { extractToken, setLocalStaorageAndUpdateState } from '../../utils/utils'
...
const CheckoutScreen = () => {
  ...
  const {cart,changeCart,jwtToken} = useContext(Context) as AppContext;
          ...
          <div className="max-w-lg mx-auto">
            ...
            <div className="relative mt-8">
              ...
              <div className="flex justify-center space-x-6 py-4">
                <div>
                    <Link to={`${ROUTES.REGISTER}`}className="text-sm font-medium hover:text-gray-400">
                      for purchase please register
                    </Link>
                    <span className="text-sm font-medium text-red-800">{` Or `}</span>
                    <Link to={`${ROUTES.SIGN_IN}`}className="text-sm font-medium hover:text-gray-400">
                      sign in
                    </Link>
                </div>
              </div>
